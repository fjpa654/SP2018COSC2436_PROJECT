 ...
 * @author francisco
 */
public class SP2018PROJECT_LQHashedEnrollment_Padilla  
{ 
    int N;
    int n = 0;			// the number of nodes in the structure
    int defaultQuotient = 9967;  	// the default 4k+3 prime
    double loadingFactor = 0.75;
    SP2018PROJECT_Enrollment_Padilla  deleted;		// the dummy node, v2
    private SP2018PROJECT_Enrollment_Padilla [] data; 	// the primary storage array

    public SP2018PROJECT_LQHashedEnrollment_Padilla (int length)
    { int pct = (int)((1.0/loadingFactor-1) *100.0);
      N = fourKPlus3(length, pct);
      data = new SP2018PROJECT_Enrollment_Padilla [N];
      deleted = new SP2018PROJECT_Enrollment_Padilla ("","","");
      for(int i = 0; i<N; i++)
         data[i] = null;
    }// end of constructor

    public boolean insert(SP2018PROJECT_Enrollment_Padilla  newSP2018PROJECT_Enrollment_Padilla)
    { boolean noError;
       boolean hit = false;
       int pass, q, offset, ip;
       int pk = stringToInt(newSP2018PROJECT_Enrollment_Padilla.getKey());  // preprocess the key
       ...
       { pass = 0;
         ...
         { data[ip]=newSP2018PROJECT_Enrollment_Padilla.deepCopy();
            n++;
            return noError = true;
         }
         else
    ...
    public SP2018PROJECT_Enrollment_Padilla fetch(String targetKey)
    {  boolean noError;
        boolean hit = false;
        int pass, q, offset, ip;
    ...
    public boolean update(String targetKey, SP2018PROJECT_Enrollment_Padilla  newSP2018PROJECT_Enrollment_Padilla )
    {  if(delete(targetKey) == false)
           return false;
       else if(insert(newSP2018PROJECT_Enrollment_Padilla ) == false)
           return false;
       return true;
    ...
    public void setAll(SP2018PROJECT_Enrollment_Padilla [] data)
    {
        this.data = data;
    }
    
    public SP2018PROJECT_Enrollment_Padilla [] getAll()
    {
        return data;
    }
